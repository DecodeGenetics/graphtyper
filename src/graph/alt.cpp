...
namespace gyper
{
Alt::Alt(std::vector<char> && _seq) : seq(std::forward<std::vector<char>>(_seq))
{
}

Alt::Alt(std::vector<char> const & _seq) : seq(_seq)
{
}

Alt::Alt(std::initializer_list<char> l) : seq(l)
{
}
...
Alt make_alt(Alt const & prev, Alt const & curr, long const jump_size)
{
  ...
  std::copy(curr.events.begin(), curr.events.end(), std::inserter(new_alt.events, new_alt.events.begin()));
