...
void
reformat_sv_vcf_records(std::vector<Variant> & variants)
{
...
//
        //for (long c = 0; c < static_cast<long>(var.seqs[0].size()); ++c)
        //{
        //  assert(c < static_cast<long>(var.seqs[a].size()));
//
        //  if (var.seqs[0][c] == var.seqs[a][c])
        //  {
        //    new_sequence.push_back(var.seqs[a][c]);
        //  }
        //  else
        //  {
        //    break;
...
//
        ...
        //std::move(sv_allele.cbegin(), sv_allele.cend(), std::back_inserter(new_sequence));
        //var.seqs[a] = std::move(new_sequence);
        sv_ids[a - 1l] = sv_id;
      }
    }
    ...
    auto add_sv_variant = [&](Variant const & var,
                              long aa,
         ...
         )
      {
        for (auto & call : new_sv_var.calls)
        {
          ...
          uint64_t gt_01 = static_cast<uint64_t>(0.499999999 +
            minus_10log10_one_third * static_cast<double>(call.coverage[1]) +
      ...
      if (graph.is_sv_graph)
      {
        if ((sv.type == DEL || sv.type == DEL_ALU))
        {
          ...
          for (long pn_index = 0; pn_index < static_cast<long>(cov_var.calls.size()); ++pn_index)
            cov_var.calls[pn_index] = make_call_based_on_coverage(pn_index, sv);
