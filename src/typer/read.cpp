...
namespace gyper
{
bool Alignment::has_indel_event(Tindel_events::iterator indel_event) const
{
...
bool Alignment::is_clipped() const
{
  return num_clipped_begin != 0 || num_clipped_end != 0;
}

void Alignment::add_indel_event(int32_t pos, uint16_t flags, uint8_t mapq, Tindel_events::iterator indel_it)
{
...
void Alignment::replace_indel_events(uint16_t flags, uint8_t mapq, std::vector<ReadIndelEvent> && new_indel_events)
{
