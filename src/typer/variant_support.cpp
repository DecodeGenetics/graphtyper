...
namespace gyper
{
void VariantSupport::set_depth(uint16_t const _depth)
{
  // Make sure that the depth is never reduced
  depth = std::max(depth, _depth);
}

double VariantSupport::get_ratio() const
{
  double const effective_depth = static_cast<long>(depth) - static_cast<long>(lq_support / 2);
...
long VariantSupport::get_score() const
{
...
double VariantSupport::get_corrected_support() const
{
...
bool VariantSupport::is_ratio_above_cutoff(double const min_ratio) const
{
  return get_ratio() > min_ratio;
}

bool VariantSupport::is_support_above_cutoff(long const min_support) const
{
  int const _depth = hq_support + lq_support;
  bool const is_promising =
    unique_positions.size() >= 3 && hq_support >= 4 && proper_pairs >= 3 && (_depth - clipped >= 3);
