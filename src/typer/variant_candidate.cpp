...
namespace gyper
{
bool VariantCandidate::add_base_in_front(bool const add_N)
{
  Variant new_var;
...
bool VariantCandidate::add_base_in_back(bool const add_N)
{
  Variant new_var;
  new_var.abs_pos = abs_pos;
  new_var.seqs = std::move(seqs);
...
void VariantCandidate::expanded_normalized()
{
  Variant new_var;
  new_var.abs_pos = abs_pos;
  new_var.seqs = std::move(seqs);
...
void VariantCandidate::normalize()
{
  Variant new_var;
  new_var.abs_pos = abs_pos;
  new_var.seqs = std::move(seqs);
...
bool VariantCandidate::is_normalized() const
{
  Variant new_var;
  new_var.abs_pos = abs_pos;
  new_var.seqs = seqs;
...
bool VariantCandidate::is_snp_or_snps() const
{
  Variant new_var;
  new_var.abs_pos = abs_pos;
  new_var.seqs = seqs;
...
int VariantCandidate::is_transition_or_transversion() const
{
  if (seqs.size() == 2 && seqs[0].size() == 1 && seqs[1].size() == 1)
  {
    if ((seqs[0][0] == 'A' && seqs[1][0] == 'G') || (seqs[0][0] == 'G' && seqs[1][0] == 'A') ||
        (seqs[0][0] == 'C' && seqs[1][0] == 'T') || (seqs[0][0] == 'T' && seqs[1][0] == 'C'))
    {
      return 1; // transition
    }
    else
...
std::string VariantCandidate::print() const
{
  Variant new_var;
  new_var.abs_pos = abs_pos;
  new_var.seqs = seqs;
...
bool VariantCandidate::operator==(VariantCandidate const & v) const
{
  return abs_pos == v.abs_pos && seqs == v.seqs;
}

bool VariantCandidate::operator!=(VariantCandidate const & b) const
{
  return !(*this == b);
}
...
std::size_t VariantCandidateHash::operator()(VariantCandidate const & v) const
{
