...
std::array<double, 11l> constexpr LOGF_ABHet{
  {-6.03446, -6.03446, -1.35948, -0.84956, -0.28956, 0.0, -1.05013, -1.35024, -1.34475, -3.74512, -3.74512}};

std::array<double, 11l> constexpr LOGF_SBAlt{
  {-0.32486, -0.32486, -0.25342, -0.32696, 0.02442, 0.0, -0.33522, -0.41332, -0.74043, -1.60844, -1.60844}};

} // namespace

namespace gyper
{
inline double get_logf(double abhom,
                       ...
                       long sbalt_bin)
{
  ...
  double const pwr = LOGF_INTERCEPT + abhom * LOGF_ABHom + cr_by_seqdepth * LOGF_CRBySeqDepth + mq * LOGF_MQ +
                     pass_ratio * LOGF_PASS_ratio + gt_yield * LOGF_GTYield + qd * LOGF_QD + LOGF_ABHet[ab_het_bin] +
                     LOGF_SBAlt[sbalt_bin];
  // BOOST_LOG_TRIVIAL(info) << __HERE__ << " log model pwr = " << pwr;
  double const _exp = std::max(0.0, std::exp(-pwr));
  return 1.0 / (1.0 + _exp);
}

inline double get_aa_score(double abhom, double sb, double mm, long sd, double qd, double cr, long mq)
{
  ...
  std::array<double, 5l> constexpr AA_ABHom{{0.0, 1.304140117, 1.681221065, 2.214801195, 3.930106559}};
  ...
  double const pwr =
    AA_INTERCEPT + AA_ABHom[abhom_bin] + sb * AA_SB + mm * AA_MM + sd * AA_SD + qd * AA_QD + cr * AA_CR + mq * AA_MQ;
